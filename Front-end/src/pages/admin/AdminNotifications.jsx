    ...
    <div className="space-y-8 max-w-6xl">
      ...
      <div className="flex flex-col sm:flex-row justify-between items-start sm:items-center bg-white dark:bg-slate-800/60 p-6 rounded-3xl border border-slate-200 dark:border-slate-700/60 shadow-[0_4px_25px_rgba(0,0,0,0.02)] gap-4">
        <div>
          ...
          <p className="text-sm text-slate-500 dark:text-slate-400 mt-1">View interviewer updates, credit allocations, and completed session alerts.</p>
        </div>
        ...
        <div className="flex items-center justify-between border-b border-slate-100 dark:border-slate-800 px-6 py-4">
          <div className="flex gap-2">
            <button
              ...
              style={filter === 'all' ? (theme === 'dark' ? { backgroundColor: 'rgba(49, 46, 129, 0.6)', color: '#818cf8' } : { backgroundColor: '#eef2ff', color: '#4f46e5' }) : {}}
            >
            ...
            <button
              ...
              style={filter === 'unread' ? (theme === 'dark' ? { backgroundColor: 'rgba(49, 46, 129, 0.6)', color: '#818cf8' } : { backgroundColor: '#eef2ff', color: '#4f46e5' }) : {}}
            >
              Unread ({unreadCount})
            </button>
          </div>
        ...
        {loading && notifications.length === 0 ? (
          ...
          <div className="divide-y divide-slate-100">
            {filteredNotifications.map((n) => (
              <div
                key={n.id}
                className={`flex items-start justify-between p-6 gap-4 transition-all hover:bg-slate-50 dark:bg-slate-900/50 dark:hover:bg-slate-700/50 ${!n.read ? 'bg-slate-50 dark:bg-slate-900/50/20' : ''
                  }`}
              >
