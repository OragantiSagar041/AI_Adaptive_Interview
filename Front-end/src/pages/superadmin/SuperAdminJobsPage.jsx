...
import { Briefcase, Plus, MapPin, Clock, FileText, X, Target, Trash2, Pencil, Wallet, Users, LayoutGrid, LayoutList, ArrowRight, ChevronRight, Zap, Building2, BookOpen, CheckCircle2 } from 'lucide-react';
  ...
  return (
    <div className="w-full max-w-7xl text-slate-800">
      ...
      {selectedJobForCandidates && (
        <div className="fixed inset-0 bg-slate-900/40 backdrop-blur-md z-50 flex items-center justify-center p-4">
          <div
            className="bg-white/95 backdrop-blur-xl rounded-[2rem] shadow-[0_20px_60px_rgba(0,0,0,0.15)] border border-white/60 w-full max-w-4xl max-h-[90vh] overflow-hidden flex flex-col"
            onClick={(e) => e.stopPropagation()}
          >
            <div className="p-7 border-b border-teal-100/50 flex items-center justify-between">
              <div className="flex items-center gap-4">
                <div className="w-12 h-12 rounded-2xl flex items-center justify-center bg-gradient-to-br from-teal-500 to-cyan-600 text-white shadow-lg">
                  <Users size={24} />
                </div>
                <div>
                  <h2 className="text-xl font-black text-slate-800">Candidates Applied</h2>
                  <p className="text-xs font-semibold text-slate-500 mt-0.5">{selectedJobForCandidates.title}</p>
                </div>
              </div>
              <button
                ...
                if (jobCandidates.length === 0) {
                  return (
                    <div className="flex flex-col items-center justify-center py-16 text-slate-400">
                      ...
                      <p className="font-semibold">No candidates yet for this role.</p>
                    </div>
                  );
                }
                ...
                return (
                  <div className="overflow-x-auto">
                    <table className="w-full text-left border-collapse">
                      <thead>
                        <tr className="border-b-2 border-slate-200">
                          <th className="py-3 px-4 text-[0.75rem] font-extrabold uppercase text-slate-400 tracking-wider">Candidate</th>
                          <th className="py-3 px-4 text-[0.75rem] font-extrabold uppercase text-slate-400 tracking-wider">Status</th>
                          <th className="py-3 px-4 text-[0.75rem] font-extrabold uppercase text-slate-400 tracking-wider">Score</th>
                          <th className="py-3 px-4 text-[0.75rem] font-extrabold uppercase text-slate-400 tracking-wider">Date</th>
                        </tr>
                      </thead>
                      <tbody>
                        {jobCandidates.map(c => (
                          <tr key={c.id || c.link_id} className="border-b border-slate-100 hover:bg-slate-50 transition-colors">
                            <td className="py-3 px-4 text-sm font-bold text-slate-800">{c.candidate_name || 'N/A'}</td>
                            <td className="py-3 px-4 text-sm">
                              <span className={`px-2.5 py-1 rounded-full text-xs font-bold border ${getComputedStatus(c) === 'Completed' ? 'bg-emerald-50 text-emerald-700 border-emerald-200' :
                                  getComputedStatus(c) === 'Pending' ? 'bg-amber-50 text-amber-700 border-amber-200' :
            ...
            <div className="p-7 border-t border-slate-100 flex justify-end">
              <button
                onClick={() => setSelectedJobForCandidates(null)}
                className="px-6 py-3 rounded-xl bg-slate-100 hover:bg-slate-200 text-slate-600 font-bold text-sm border-none cursor-pointer transition-colors"
              >
