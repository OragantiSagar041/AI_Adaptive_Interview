...
@celery_app.task(name="tasks.score_answer_task")
def score_answer_task(interview_id: str, question_id: int, question_text: str, answer_text: str, context: str, time_spent_seconds: int, time_limit_seconds: int, language: str):
    try:
...
def send_email_task(candidate_email: str, candidate_name: str, link_url: str, duration: int, job_description: str, custom_html: str = "", scheduled_start: str = "", scheduled_end: str = ""):
    ...
    try:
        ...
        payload = {
            "sender": {"name": "HireIQ Recruiting", "email": os.getenv("BREVO_SENDER_EMAIL", "oragantisagar041@gmail.com")},
...
def generate_report_task(interview_id: str):
    ...
    from main import generate_report # We can call the main.py function safely inside the task
    try:
