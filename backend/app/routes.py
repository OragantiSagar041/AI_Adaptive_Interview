...
def get_interview_details(link_id: str):
    ...
    # If there are no results, forcefully zero out all scores to prevent old cache from lingering
    if not results:
        recommendation = "No Data"
        strengths = "No answers provided."
        weaknesses = "No answers provided."
        ...
        try:
            ...
            print(f"Zero cache error: {e}")

    # Use cached values if available and results exist
    elif saved_rec and saved_comm is not None and saved_skills is not None:
...
def reschedule_session(link_id: str, new_expiry: str = Form(...), new_start: str = Form(None), current_admin: dict = Depends(require_role("admin", "super_admin"))):
    ...
    # Generate a custom reschedule email
    html_content = f"""
    <div style="font-family: Arial, sans-serif; padding: 20px; max-width: 600px; border: 1px solid #e2e8f0; border-radius: 10px;">
        ...
        <p>Hi {updated_session.get('candidate_name', 'Candidate')},</p>
        <p>Your AI Mock Interview session has been rescheduled by the administrative team.</p>
        <p><strong>New Start Time:</strong> {new_start or 'Available immediately'}</p>
        <p><strong>New Expiry Time:</strong> {new_expiry}</p>
        <p style="margin-top: 20px;">Please use the button below to access your interview when it is time:</p>
        <a href="{link_url}" style="display:inline-block; margin-top: 10px; padding:12px 24px; background-color:#4F46E5; color:white; font-weight:bold; text-decoration:none; border-radius:8px;">Access Interview</a>
        <p style="margin-top: 30px; font-size: 12px; color: #64748b;">If you have any questions, please contact the recruiter.</p>
    </div>
    """
    ...
    try:
        ...
        # Call synchronously so it sends right away
        res = send_email_task(
            candidate_email=updated_session.get("candidate_email", ""),
            candidate_name=updated_session.get("candidate_name", ""),
            link_url=link_url,
            duration=updated_session.get("interview_duration", 30),
            job_description=updated_session.get("job_description", ""),
            custom_html=html_content,
            scheduled_start=new_start or "",
            scheduled_end=new_expiry or ""
        )
        email_sent = True if res.get("status") in [200, 201, 202] else False
    except Exception as e:
...
def update_interview_notes(link_id: str, payload: NotesPayload, current_admin: dict = Depends(require_role("admin", "super_admin"))):
    interview_sessions_collection.update_one(
        {"link_id": link_id},
...
async def live_heartbeat(data: LiveHeartbeatRequest):
    ...
    import json
    if manager.redis:
        # If using Redis, fetch existing and merge
        val = await manager.redis.get(f"live_snapshot:{data.link_id}")
        if val:
            existing = json.loads(val)
            snap_data = {**existing, **snap_data}
            if "round_type" not in snap_data and data.round_type:
                snap_data["round_type"] = data.round_type
        ...
        existing = _live_snapshots.get(data.link_id, {})
        snap_data = {**existing, **snap_data}
        if data.round_type:
            snap_data["round_type"] = data.round_type
                ...
                if data.get("type") == "telemetry":
                    ...
                    globals()["_live_snapshots"][link_id] = {
                        **existing_snapshot,
                        ...
                        "round_type": telemetry_payload.get("round_type", existing_snapshot.get("round_type", "")),
...
async def get_dashboard_aggregated_data(admin_id: Optional[str] = None, current_admin: dict = Depends(get_current_admin_details)):
    try:
        ...
        if has_live or current_admin.get("role") in ["master", "super_admin"]:
            ...
            rows = list(interview_sessions_collection.find(
                query_filter,
                {"link_id": 1, "candidate_name": 1, "candidate_email": 1, "created_at": 1, "interview_id": 1, "started_at": 1, "interview_title": 1, "job_applied": 1}
            ).sort("created_at", -1))
...
def bulk_delete_candidates(data: BulkDeleteRequest, current_admin: dict = Depends(get_current_admin_details)):
    try:
        ...
        for id_str in data.ids:
            if id_str.startswith("ai_call_"):
                app_id = id_str.replace("ai_call_", "")
                from bson import ObjectId
                try:
                    job_applications_collection.delete_one({"_id": ObjectId(app_id)})
                    deleted_count += 1
                except:
                    pass
                continue
