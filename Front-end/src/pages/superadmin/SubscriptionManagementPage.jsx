...
const STATUS_CONFIG = {
  ...
  blocked: { label: "Blocked", bg: "bg-slate-100 dark:bg-slate-800/50",  text: "text-slate-600 dark:text-slate-400",   border: "border-slate-200 dark:border-slate-700",   dot: "bg-slate-400",   Icon: XCircle },
};

const PLAN_COLORS = {
  trial:   { bg: "bg-slate-100 dark:bg-slate-800/50",  text: "text-slate-700 dark:text-slate-200"  },
  ...
  return (
    <div className="px-6 pb-6 grid grid-cols-1 md:grid-cols-2 gap-6 mt-2">
      <div className="bg-slate-50 dark:bg-slate-900/50 rounded-xl p-4">
        <p className="text-xs font-semibold text-slate-500 dark:text-slate-400 uppercase tracking-wider mb-3">Plan Features</p>
      ...
      <div className="bg-slate-50 dark:bg-slate-900/50 rounded-xl p-4">
        <p className="text-xs font-semibold text-slate-500 dark:text-slate-400 uppercase tracking-wider mb-3">Recharge History</p>
        {detail.recharge_history?.length ? (
          <ul className="space-y-2">
            {detail.recharge_history.map((r, i) => (
              <li key={i} className="flex justify-between items-center text-sm">
                <span className="text-slate-700 dark:text-slate-200 font-medium">{r.plan_name}</span>
                <div className="text-right">
                  <p className="font-semibold text-slate-800 dark:text-slate-100">₹{r.amount ?? 0}</p>
...
function SubscriptionCard({ s, onRecharge }) {
  ...
  return (
    <div className="bg-white dark:bg-slate-800/60 rounded-2xl border border-slate-100 dark:border-slate-800 shadow-sm hover:shadow-md transition-shadow overflow-hidden">
      {/* Top: owner info + status */}
      <div className="px-6 pt-6 pb-4 flex items-start justify-between gap-4">
        <div className="flex items-center gap-4 min-w-0">
          <Avatar name={s.company_name || s.owner_name} />
          <div className="min-w-0">
            <p className="text-lg font-bold text-slate-800 dark:text-slate-100 truncate leading-tight">
              {s.company_name || s.owner_name}
            </p>
            {s.primary_email && (
              <div className="flex items-center gap-1.5 mt-0.5">
                <Mail className="w-3 h-3 text-slate-400 shrink-0" />
                <p className="text-xs text-slate-500 dark:text-slate-400 truncate">{s.primary_email}</p>
              </div>
            )}
            <div className="mt-2">
      ...
      <div className="px-6 py-4 grid grid-cols-2 md:grid-cols-4 gap-4">
        ...
        <div>
          ...
          <p className="text-xl font-bold text-slate-800 dark:text-slate-100">{s.credits_available.toLocaleString()}</p>
        ...
        <div>
          ...
          <p className="text-base font-bold text-slate-800 dark:text-slate-100">{s.renews_on ?? "—"}</p>
        ...
        <div>
          ...
          <p className="text-xl font-bold text-slate-800 dark:text-slate-100">{creditsUsed.toLocaleString()}</p>
      ...
      <button
        ...
        className="w-full flex items-center justify-center gap-1.5 py-2.5 text-xs text-slate-400 hover:text-slate-600 dark:text-slate-400 hover:bg-slate-50 dark:bg-slate-900/50 dark:hover:bg-slate-700 transition-colors border-t border-slate-100 dark:border-slate-800"
      >
  ...
  return (
    <AdminShell title="Subscription Management" description="Plans, renewals and billing for every organization.">
      ...
      <div className="flex items-center justify-between mb-4">
        <div>
          <h2 className="text-sm font-semibold text-slate-700 dark:text-slate-200">Active subscriptions</h2>
          <p className="text-xs text-slate-400">{subs.length} organisation{subs.length !== 1 ? "s" : ""}</p>
        </div>
        <Button size="sm" variant="ghost" className="gap-2 text-xs text-slate-500 dark:text-slate-400" onClick={() => fetchData(true)} disabled={refreshing}>
