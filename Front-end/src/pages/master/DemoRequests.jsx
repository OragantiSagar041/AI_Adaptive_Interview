...
import { Mail, Briefcase, Calendar, CheckCircle, Trash2, MessageSquare, Send, X, Sparkles, User, Building } from 'lucide-react';
...
HireIQ Client Success Team`
  ...
  const handleSendEmail = async (e) => {
    e.preventDefault();
    if (!selectedReq || !selectedReq.work_email) {
      Swal.fire({
    ...
    try {
      const response = await fetch(`${API_BASE_URL}/master/demo-requests/${selectedReq.id}/send-email`, {
        method: 'POST',
        headers: {
          'Content-Type': 'application/json',
          'Authorization': `Bearer ${token}`
        ...
        body: JSON.stringify({
          recipient_email: selectedReq.work_email,
      ...
      if (response.ok && data.status === 'success') {
        ...
        setRequests(requests.map(r => 
          r.id === selectedReq.id ? { ...r, status: 'CONTACTED' } : r
        ));
        setIsEmailModalOpen(false);
        Swal.fire({
          title: 'Email Sent!',
          text: `Your message has been delivered to ${selectedReq.work_email}. Lead marked as CONTACTED.`,
          icon: 'success',
          background: '#161c2d',
          color: '#fff',
        });
      ...
      Swal.fire({
        ...
        text: error.message || 'Could not send email to client.',
        icon: 'error',
        background: '#161c2d',
        color: '#fff',
      });
        ...
        <div>
          <h2 className="text-2xl font-bold text-slate-800 mb-1 flex items-center gap-2.5">
            ...
            <span className="text-xs font-semibold px-2.5 py-0.5 rounded-full bg-indigo-50 text-indigo-600 border border-indigo-200">
              {requests.length} Leads
            </span>
          </h2>
          <p className="text-sm text-slate-500">Manage incoming demo inquiries and email prospective clients directly</p>
        </div>
        <button 
              ...
              {loading ? (
                <tr>
                  <td colSpan="6" className="p-12 text-center text-slate-500">
                    ...
                    <span className="text-sm font-medium">Loading demo inquiries...</span>
                  </td>
                </tr>
              ) : requests.length === 0 ? (
                <tr>
                  <td colSpan="6" className="p-12 text-center text-slate-400">
                    ...
                    <p className="text-sm font-medium">No demo requests received yet.</p>
                  </td>
                </tr>
              ) : (
                requests.map((req) => (
                  <tr key={req.id} className="hover:bg-slate-50/70 transition-colors group">
                    ...
                    <td className="p-4">
                      <div className="flex items-start gap-2 text-sm text-slate-600 bg-slate-50/60 p-2.5 rounded-xl border border-slate-100">
                        ...
                        <span className="line-clamp-2 text-xs leading-relaxed" title={req.help_text}>
                          {req.help_text || 'No message provided.'}
                        </span>
                      </div>
                    </td>
                    ...
                    <td className="p-4 pr-6 text-right">
                      <div className="flex items-center justify-end gap-2">
                        ...
                        <button 
                          ...
                          title={`Send Email to ${req.work_email}`}
                          className="flex items-center gap-1.5 px-3 py-1.5 rounded-xl bg-indigo-50 hover:bg-indigo-600 text-indigo-600 hover:text-white border border-indigo-200 hover:border-indigo-600 font-semibold text-xs transition-all shadow-xs cursor-pointer group/btn"
                        >
            ...
            <div className="flex justify-between items-start border-b border-slate-100 pb-4">
              <div className="flex items-center gap-3">
                ...
                <div>
                  ...
                  <p className="text-xs text-slate-500 mt-0.5">Reach out directly to schedule a demo and answer client questions</p>
                </div>
              </div>
              <button
                type="button"
...
HireIQ Demo Team`
              ...
              <div className="flex items-center justify-end gap-3 pt-4 border-t border-slate-100">
                ...
                <button
                  type="submit"
                  disabled={sendingEmail}
                  className="flex items-center gap-2 px-6 py-2.5 rounded-xl bg-indigo-600 hover:bg-indigo-500 text-white font-bold text-sm shadow-md shadow-indigo-200 cursor-pointer disabled:opacity-50 transition-all border-none"
                >
                  {sendingEmail ? (
                    <>
