    ...
    <div className="min-h-[calc(100vh-64px)] bg-background text-foreground">
      <main className="mx-auto max-w-7xl px-4 sm:px-6 py-8 space-y-8">
        ...
        <div className="flex flex-col sm:flex-row sm:items-center justify-between gap-4">
          <div>
            <h1 className="text-3xl font-black tracking-tight text-foreground">Qualified Candidates</h1>
          ...
          <button
            ...
            className="inline-flex items-center gap-2 bg-card border border-border text-foreground px-4 py-2 rounded-xl font-bold shadow-sm hover:bg-muted transition-all cursor-pointer"
          >
        ...
        <section className="grid grid-cols-2 md:grid-cols-3 lg:grid-cols-6 gap-4">
          {STATS.map((s) => (
            <div
              key={s.label}
              className="rounded-2xl border border-border/60 bg-card p-5 shadow-sm hover:shadow-md transition-shadow flex flex-col items-start"
            >
        ...
        <section className="rounded-2xl border border-border/60 bg-card p-5 shadow-sm">
          <div className="flex items-center gap-2 mb-4">
            ...
            <span className="text-sm font-bold text-foreground uppercase tracking-wider">Search & Filters</span>
          </div>
          <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-6 gap-4">
            <div className="relative lg:col-span-2">
              ...
              <input
                ...
                className="w-full pl-9 pr-4 py-2.5 rounded-xl border border-border bg-background text-sm font-medium text-foreground outline-none focus:border-indigo-500 focus:ring-2 focus:ring-indigo-500/20 transition-all"
              />
            </div>
            ...
            <div className="relative">
              <select
                ...
                className="w-full px-4 py-2.5 rounded-xl border border-border bg-background text-sm font-medium text-foreground outline-none focus:border-indigo-500 focus:ring-2 focus:ring-indigo-500/20 transition-all cursor-pointer appearance-none"
              >
            ...
            <div className="relative">
              <input
                ...
                className="w-full px-4 py-2.5 rounded-xl border border-border bg-background text-sm font-medium text-foreground outline-none focus:border-indigo-500 focus:ring-2 focus:ring-indigo-500/20 transition-all cursor-pointer"
              />
            </div>
          </div>
        </section>
        ...
        <section className="rounded-2xl border border-border/60 bg-card shadow-sm overflow-hidden">
          <div className="flex items-center justify-between p-5 border-b border-border/70 bg-muted/30">
            <div className="text-sm">
              ...
              <span className="text-muted-foreground font-medium"> candidates</span>
            </div>
          </div>
          ...
          <div className="overflow-x-auto">
            <table className="w-full text-left border-collapse min-w-[900px]">
              <thead>
                <tr className="border-b border-border/70 bg-muted/10 whitespace-nowrap">
                  <th className="py-4 px-5 text-xs font-bold text-muted-foreground uppercase tracking-wider">Candidate</th>
                  <th className="py-4 px-5 text-xs font-bold text-muted-foreground uppercase tracking-wider">Job Applied</th>
                  <th className="py-4 px-5 text-xs font-bold text-muted-foreground uppercase tracking-wider">AI Score</th>
                  <th className="py-4 px-5 text-xs font-bold text-muted-foreground uppercase tracking-wider">Skills Match</th>
                  <th className="py-4 px-5 text-xs font-bold text-muted-foreground uppercase tracking-wider">Interview</th>
                  <th className="py-4 px-5 text-xs font-bold text-muted-foreground uppercase tracking-wider">Assigned Recruiter</th>
                  <th className="py-4 px-5 text-xs font-bold text-muted-foreground uppercase tracking-wider">Interview Date & Time</th>
                  <th className="py-4 px-5 text-xs font-bold text-muted-foreground uppercase tracking-wider">Status</th>
                  <th className="py-4 px-5 text-xs font-bold text-muted-foreground uppercase tracking-wider text-right">Actions</th>
                </tr>
              </thead>
              <tbody className="divide-y divide-border/60">
                {filtered.map((c) => {
                  ...
                  return (
                    <tr key={c.id || c.link_id || c.email} onClick={() => setSelectedCandidate(c)} className="hover:bg-muted/30 transition-colors group whitespace-nowrap cursor-pointer">
                      <td className="px-5 py-4">
                        <div className="flex items-center gap-3">
                          <div className="flex h-10 w-10 items-center justify-center rounded-xl bg-indigo-100/20 text-indigo-300 font-bold shrink-0 shadow-sm border border-indigo-500/10">
                            {(c.candidate_name || "U")[0].toUpperCase()}
                          </div>
                          <div>
                            <div className="font-bold text-foreground text-sm">{c.candidate_name}</div>
                            <div className="text-xs font-medium text-muted-foreground mt-0.5">{c.candidate_email || c.email || 'No email provided'}</div>
                          </div>
                        </div>
                      </td>
                      <td className="px-5 py-4 text-sm font-semibold text-foreground">{c.interview_title || c.job_applied}</td>
                      <td className="px-5 py-4 text-sm font-bold text-foreground tabular-nums">{scoreNum.toFixed(0)}%</td>
                      ...
                      <td className="px-5 py-4 text-sm font-medium text-muted-foreground">{adminName}</td>
                      <td className="px-5 py-4 text-sm font-medium text-muted-foreground">
                        {c.created_at ? (
                          <div className="flex flex-col">
                            <span className="text-foreground font-semibold">{new Date(c.created_at).toLocaleDateString(undefined, { month: 'short', day: 'numeric', year: 'numeric' })}</span>
                            <span className="text-[11px] text-muted-foreground mt-0.5">{new Date(c.created_at).toLocaleTimeString(undefined, { hour: '2-digit', minute: '2-digit' })}</span>
                          </div>
                        ) : 'N/A'}
                      </td>
                      <td className="px-5 py-4">
                      ...
                      <td className="px-5 py-4 text-right">
                        <button
                          ...
                          className="inline-flex items-center gap-1.5 px-3 py-1.5 rounded-lg text-xs font-bold bg-muted/20 border border-border text-foreground hover:text-indigo-400 hover:border-indigo-300 hover:bg-indigo-500/10 shadow-sm transition-all cursor-pointer"
                        >
