  ...
  return createPortal(
    ...
    >
      <div 
        ...
        className={`bg-white dark:bg-slate-800/60 border border-slate-200 dark:border-slate-700 rounded-2xl w-full ${selectedConfig ? 'max-w-xl' : 'max-w-6xl'} overflow-hidden shadow-2xl flex flex-col max-h-[88vh] my-auto transition-all duration-300 relative`}
      >
        
        {/* Dynamic Header */}
        <div className="flex items-center justify-between px-6 py-4.5 border-b border-slate-200 dark:border-slate-700 bg-slate-50 dark:bg-slate-900/50 shrink-0 sticky top-0 z-20">
          <div className="flex items-center gap-3">
            {selectedConfig ? (
              <button 
                onClick={() => {setSelectedConfig(null); setError(''); setSuccess('')}} 
                className="p-2 text-slate-500 dark:text-slate-400 hover:text-slate-900 dark:text-white rounded-xl bg-white dark:bg-slate-800/60 hover:bg-slate-100 dark:bg-slate-800/50 transition-colors border border-slate-200 dark:border-slate-700"
            ...
            <div>
              <h2 id="integration-modal-title" className="text-lg font-bold text-slate-900 dark:text-white tracking-wide leading-tight">
          ...
          <button 
            onClick={handleClose} 
            className="p-2 text-slate-500 dark:text-slate-400 hover:text-slate-900 dark:text-white rounded-xl bg-white dark:bg-slate-800/60 hover:bg-slate-100 dark:bg-slate-800/50 border border-slate-200 dark:border-slate-700 transition-all hover:scale-105 active:scale-95 shadow-sm flex items-center justify-center cursor-pointer"
            title="Close modal"
          >
            <X size={20} />
          </button>
            ...
            <div className="space-y-6">
              ...
              <div className="flex flex-col md:flex-row items-stretch md:items-center justify-between gap-4 pb-2 border-b border-white/5">
                    ...
                    <button
                      ...
                      className={`px-3.5 py-1.5 rounded-full text-xs font-semibold tracking-wide border transition-all cursor-pointer ${
                        activeCategory === cat.id 
                          ...
                          : 'bg-white dark:bg-slate-800/60 border-slate-200 dark:border-slate-700 text-slate-500 dark:text-slate-400 hover:border-white/30 hover:text-slate-900 dark:text-white hover:bg-slate-100 dark:bg-slate-800/50'
                      }`}
                    >
                      {cat.id} <span className="opacity-60 ml-1 text-[0.68rem] bg-slate-100 dark:bg-slate-800/50 px-1.5 py-0.5 rounded-full">{cat.count}</span>
                    </button>
                  ))}
                </div>
                <div className="relative w-full md:w-64 shrink-0">
                  ...
                  <input 
                    ...
                    className="w-full bg-slate-50 dark:bg-slate-900/50 border border-slate-200 dark:border-slate-700 rounded-xl pl-9 pr-8 py-2 text-xs text-slate-900 dark:text-white placeholder-slate-500 focus:outline-none focus:border-indigo-500/80 transition-all"
                  />
                  {searchQuery && (
              ...
              <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-4 sm:gap-5">
                {filteredIntegrations.map((int, idx) => (
                  <div key={int.id || idx} className="bg-slate-50 dark:bg-slate-900/50 border border-slate-200 dark:border-slate-700 hover:border-indigo-500/40 rounded-xl overflow-hidden flex flex-col group hover:shadow-lg hover:shadow-indigo-500/5 transition-all duration-200">
                    <div className="p-5 flex-1 flex flex-col">
                      <div className="flex items-start justify-between mb-4">
                        <div className="flex items-center gap-3">
                          <div className={`p-2.5 rounded-xl ${int.bg} border border-slate-200 dark:border-slate-700 group-hover:scale-105 transition-transform flex items-center justify-center shrink-0`}>
                            <int.icon className="w-6 h-6" />
                          </div>
                          <div>
                            <span className="font-bold text-slate-900 dark:text-white tracking-wide block text-sm">{int.name}</span>
                            <span className="text-[0.7rem] text-slate-500 dark:text-slate-400 block mt-0.5">{int.category}</span>
                          </div>
                        </div>
                    ...
                    <div className="border-t border-white/5 p-3.5 px-5 flex justify-end bg-white dark:bg-slate-800/60/[0.02]">
                      <button 
                        onClick={() => setSelectedConfig(int.id)}
                        className="flex items-center gap-1.5 px-4 py-1.5 bg-indigo-600/90 hover:bg-indigo-600 border border-indigo-500/30 rounded-lg text-xs font-bold text-slate-900 dark:text-white transition-all shadow-sm group-hover:shadow-indigo-500/20 cursor-pointer"
                      >
                        Connect <ExternalLink size={12} className="text-indigo-200 group-hover:text-slate-900 dark:text-white transition-colors" />
                      </button>
                    </div>
                  </div>
                ))}
            ...
            <div className="space-y-6">
              ...
              {selectedConfig === 'calendly' && (
                <form onSubmit={handleCalendlySubmit} className="space-y-5">
                  ...
                  <div>
                    ...
                    <input required value={calendlyForm.name} onChange={e => setCalendlyForm({ ...calendlyForm, name: e.target.value })} placeholder="Integration name" className="w-full px-4 py-3 bg-slate-50 dark:bg-slate-900/50 border border-slate-200 dark:border-slate-700 rounded-xl text-xs text-slate-900 dark:text-white placeholder-slate-600 focus:outline-none focus:border-indigo-500" />
                  </div>
                  <div>
                    ...
                    <input
                      ...
                      className="w-full px-4 py-3 bg-slate-50 dark:bg-slate-900/50 border border-slate-200 dark:border-slate-700 rounded-xl text-xs text-slate-900 dark:text-white placeholder-slate-600 focus:outline-none focus:border-indigo-500 transition-colors"
                    />
                    <p className="text-[0.65rem] text-slate-500 dark:text-slate-400 mt-2">
                  ...
                  <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
                    <div>
                      ...
                      <input required value={calendlyForm.eventTypeId} onChange={e => setCalendlyForm({ ...calendlyForm, eventTypeId: e.target.value })} placeholder="Event Type ID" className="w-full px-4 py-3 bg-slate-50 dark:bg-slate-900/50 border border-slate-200 dark:border-slate-700 rounded-xl text-xs text-slate-900 dark:text-white placeholder-slate-600 focus:outline-none focus:border-indigo-500" />
                    </div>
                    <div>
                      <label className="block text-xs font-bold text-slate-300 mb-2 uppercase tracking-wide">Timezone</label>
                      <input value={calendlyForm.timezone} onChange={e => setCalendlyForm({ ...calendlyForm, timezone: e.target.value })} placeholder="Timezone" className="w-full px-4 py-3 bg-slate-50 dark:bg-slate-900/50 border border-slate-200 dark:border-slate-700 rounded-xl text-xs text-slate-900 dark:text-white placeholder-slate-600 focus:outline-none focus:border-indigo-500" />
                    </div>
                  </div>
                  <div className="pt-4 flex justify-end">
                    <button
                      ...
                      className="flex items-center gap-2 px-6 py-2.5 bg-indigo-600 hover:bg-indigo-500 disabled:opacity-50 text-slate-900 dark:text-white font-bold text-xs rounded-xl transition-all shadow-lg shadow-indigo-600/30 cursor-pointer"
                    >
              ...
              {selectedConfig === 'cal_com' && (
                <form onSubmit={handleCalComSubmit} className="space-y-5">
                  <p className="text-xs text-slate-500 dark:text-slate-400 leading-relaxed">Connect Cal.com so the Omni assistant can check availability and schedule appointments.</p>
                  <div>
                    <label className="block text-xs font-bold text-slate-300 mb-2 uppercase tracking-wide">Integration Name *</label>
                    <input required value={calComForm.name} onChange={e => setCalComForm({ ...calComForm, name: e.target.value })} placeholder="Integration name" className="w-full px-4 py-3 bg-slate-50 dark:bg-slate-900/50 border border-slate-200 dark:border-slate-700 rounded-xl text-xs text-slate-900 dark:text-white placeholder-slate-600 focus:outline-none focus:border-indigo-500" />
                  </div>
                  <div>
                    <label className="block text-xs font-bold text-slate-300 mb-2 uppercase tracking-wide">API Key *</label>
                    <input required type="password" value={calComForm.apiKey} onChange={e => setCalComForm({ ...calComForm, apiKey: e.target.value })} placeholder="Cal.com API key" className="w-full px-4 py-3 bg-slate-50 dark:bg-slate-900/50 border border-slate-200 dark:border-slate-700 rounded-xl text-xs text-slate-900 dark:text-white placeholder-slate-600 focus:outline-none focus:border-indigo-500" />
                  </div>
                  <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
                    <div>
                      <label className="block text-xs font-bold text-slate-300 mb-2 uppercase tracking-wide">Event Type ID *</label>
                      <input required value={calComForm.eventTypeId} onChange={e => setCalComForm({ ...calComForm, eventTypeId: e.target.value })} placeholder="Event Type ID" className="w-full px-4 py-3 bg-slate-50 dark:bg-slate-900/50 border border-slate-200 dark:border-slate-700 rounded-xl text-xs text-slate-900 dark:text-white placeholder-slate-600 focus:outline-none focus:border-indigo-500" />
                    </div>
                    <div>
                      <label className="block text-xs font-bold text-slate-300 mb-2 uppercase tracking-wide">Timezone</label>
                      <input value={calComForm.timezone} onChange={e => setCalComForm({ ...calComForm, timezone: e.target.value })} placeholder="Timezone" className="w-full px-4 py-3 bg-slate-50 dark:bg-slate-900/50 border border-slate-200 dark:border-slate-700 rounded-xl text-xs text-slate-900 dark:text-white placeholder-slate-600 focus:outline-none focus:border-indigo-500" />
                    </div>
                  </div>
                  <div className="pt-4 flex justify-end">
                    <button type="submit" disabled={loading} className="flex items-center gap-2 px-6 py-2.5 bg-indigo-600 hover:bg-indigo-500 disabled:opacity-50 text-slate-900 dark:text-white font-bold text-xs rounded-xl cursor-pointer">
                ...
                <form onSubmit={handleWebhookSubmit} className="space-y-5">
                  ...
                  <div>
                    ...
                    <input
                      ...
                      className="w-full px-4 py-3 bg-slate-50 dark:bg-slate-900/50 border border-slate-200 dark:border-slate-700 rounded-xl text-xs text-slate-900 dark:text-white focus:outline-none focus:border-indigo-500 transition-colors"
                    />
                  </div>
                  <div className="grid grid-cols-4 gap-4">
                    <div className="col-span-1">
                      ...
                      <select
                        ...
                        className="w-full px-3 py-3 bg-slate-50 dark:bg-slate-900/50 border border-slate-200 dark:border-slate-700 rounded-xl text-xs text-slate-900 dark:text-white focus:outline-none focus:border-indigo-500 cursor-pointer"
                      >
                        <option>POST</option>
                        <option>GET</option>
                        <option>PUT</option>
                  ...
                  <div>
                    ...
                    <textarea
                      ...
                      className="w-full px-4 py-3 bg-slate-50 dark:bg-slate-900/50 border border-slate-200 dark:border-slate-700 rounded-xl text-xs text-slate-900 dark:text-white font-mono h-24 focus:outline-none focus:border-indigo-500 resize-none"
                    />
                  </div>
                  <div>
              ...
              {!['calendly', 'cal_com', 'custom_api'].includes(selectedConfig) && (
                <form onSubmit={handleGenericSubmit} className="space-y-5">
                  <p className="text-xs text-slate-500 dark:text-slate-400 leading-relaxed">Paste the Omni Dimension integration payload for {INTEGRATIONS.find(item => item.id === selectedConfig)?.name || 'this integration'}. The payload is sent directly to the active Omni account.</p>
                  <textarea
                    required
                    value={genericForm}
                    onChange={event => setGenericForm(event.target.value)}
                    placeholder={'{"name":"My integration","type":"...","config":{}}'}
                    className="w-full min-h-56 px-4 py-3 bg-slate-50 dark:bg-slate-900/50 border border-slate-200 dark:border-slate-700 rounded-xl text-xs text-slate-900 dark:text-white font-mono focus:outline-none focus:border-indigo-500 resize-y"
                  />
                  <div className="pt-4 flex justify-end">
                    <button type="submit" disabled={loading || !genericForm.trim()} className="flex items-center gap-2 px-6 py-2.5 bg-indigo-600 hover:bg-indigo-500 disabled:opacity-50 text-slate-900 dark:text-white font-bold text-xs rounded-xl cursor-pointer">
