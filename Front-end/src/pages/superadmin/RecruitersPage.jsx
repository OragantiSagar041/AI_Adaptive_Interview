...
export default function RecruitersPage() {
  ...
  useEffect(() => {
    ...
    const handleWheel = (e) => {
      // If standard vertical mouse wheel spin occurs over the table, translate to horizontal scroll
  ...
  const fetchStats = async () => {
    try {
      ...
      let res = null;
      try {
        res = await axios.get(`${API_BASE_URL}/api/superadmin/recruiters/stats`, { headers });
      } catch (err1) {
        try {
          res = await axios.get(`${API_BASE_URL}/superadmin/recruiters/stats`, { headers });
        } catch (err2) {
          res = null;
        }
      }

      if (res?.data?.recruiters && res.data.recruiters.length > 0) {
        setData(res.data);
      } else {
        // Fallback to /super-admin/admins (the endpoint used by Credit Management)
        const adminsRes = await axios.get(`${API_BASE_URL}/super-admin/admins`, { headers });
        const adminList = adminsRes.data?.data || [];
        const mappedRecruiters = adminList.map(a => ({
          id: a.id || a._id,
          name: a.name || a.username || a.email || 'Recruiter',
          username: a.username || a.name || 'Recruiter',
          email: a.email || 'N/A',
          role: a.role === 'super_admin' ? 'super_admin' : 'admin',
          status: (a.is_active !== false && a.login_enabled !== false) ? 'Active' : 'Inactive',
          credits: a.credits || 0,
          total_allocated_credits: a.total_allocated_credits || a.credits || 0,
          interviews_conducted: a.sessions_created || 0,
          last_active: a.last_active || a.created_at || new Date().toISOString()
        }));

        const totalRecruiters = mappedRecruiters.length;
        const activeNow = mappedRecruiters.filter(r => r.status === 'Active').length;
        const totalInterviews = mappedRecruiters.reduce((acc, r) => acc + (r.interviews_conducted || 0), 0);
        ...
        setData({
          ...
          kpis: {
            ...
            avg_interviews: roundTo(totalInterviews / Math.max(totalRecruiters, 1), 1)
          },
          recruiters: mappedRecruiters,
          weekly_activity: res?.data?.weekly_activity || []
        });
      }
    } catch (error) {
    ...
    <div className="p-6 lg:p-8 w-full max-w-7xl mx-auto space-y-8">
      ...
      <div className="w-full bg-white rounded-2xl shadow-xs border border-slate-100 overflow-hidden">
        ...
        <div 
          ...
          className="w-full overflow-x-auto max-w-full touch-pan-x relative z-10 focus:outline-none"
        >
